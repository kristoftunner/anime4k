...
def calc_psnr(img1, img2):
    return 10. * np.log10(1. / np.mean.mean((img1 - img2) ** 2))
...
def show_image(img, width=None, height=None, fmt='png'):
    ...
    if height is None:
        height = img.shape[-3]
    f = BytesIO()
    Image.fromarray(np.clip(np.array(img), 0, 255).astype(
        np.uint8)).save(f, fmt)
    IPython.display.display(IPython.display.Image(
        data=f.getvalue(), width=width, height=height))
...
def show_images(images, show_max=5, scale=1, val_range=[0, 255]):
    ...
    # Normalize images
    images = np.clip(
        ((images - val_range[0]) / (val_range[1] - val_range[0])) * 255, 0, 255).astype(np.uint8)
...
def preview_dataset(dataset, show_max=5, scale=1, normalize_scale=True, val_range=[0, 255]):
    # Take from dataset, might be slow and unoptimized if dataset is not created from generator...
    dataset_example = list(dataset.prefetch(tf.data.AUTOTUNE).take(show_max))

    target_size = recursive_max_horizontal_size(
        dataset_example) if normalize_scale else None
...
def preview_dataset_list(dataset_list, show_max, scale, target_size, val_range):
    ...
    # Dataset does not give out a tensor
        ...
        for k in range(len(dataset_example)):
            ...
            preview_dataset_list(sublist, show_max, scale,
                                 target_size, val_range)
...
def gaussian_blur_no_pad(inp, shape=(7, 7), sigma=1.0):
    ...
    conv = tf.nn.depthwise_conv2d(
        inp, k, strides=[1, 1, 1, 1], padding="VALID")
...
def degrade_ring(img, sigma, shape=(7, 7), do_clip=True):
    ...
    img = tf.pad(img, [[shape[0]//2, shape[0]//2],
                 [shape[1]//2, shape[1]//2], [0, 0]], mode="REFLECT")
...
def degrade_noise_gaussian(img, sigma, color=True, low_freq_sigma=None, low_freq_shape=(7, 7), do_clip=True):
    ...
    if color:
        ...
        rg = tf.random.normal(
            imgshape[:-1], mean=0, stddev=rs)[..., tf.newaxis]

    # Low frequency blur if requested
    if low_freq_sigma is not None:
        rg = degrade_blur_gaussian(
            rg, low_freq_sigma, low_freq_shape, do_clip=False)
...
def degrade_rgb_to_yuv(img, jpeg_factor=None, chroma_subsampling=True, chroma_method="area", do_clip=True):
    ...
    if chroma_subsampling:
        img_u = tf.image.resize(
            img_u, [yuvshape[-3]//2, yuvshape[-2]//2], method=chroma_method)
        img_v = tf.image.resize(
            img_v, [yuvshape[-3]//2, yuvshape[-2]//2], method=chroma_method)
...
def degrade_yuv_to_rgb(img, chroma_method="bicubic", do_clip=True):
    ...
    if yshape[-3] != uvshape[-3] or yshape[-2] != uvshape[-2]:
        img_uv = tf.image.resize(
            img_uv, [yshape[-3], yshape[-2]], method=chroma_method)
